<x-action-section>
    ...
    <x-slot name="content">
        ...
        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('Lorsque l\'authentification à deux facteurs est activée, vous serez invité à fournir un jeton sécurisé et aléatoire lors de l\'authentification. Vous pouvez obtenir ce jeton depuis l\'application Google Authenticator de votre téléphone.') }}
            </p>
        </div>
        ...
        @if ($this->enabled)
            ...
            @if ($showingRecoveryCodes)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        {{ __('Enregistrez ces codes de récupération dans un gestionnaire de mots de passe sécurisé. Ils peuvent être utilisés pour récupérer l\'accès à votre compte si vous perdez votre dispositif d\'authentification à deux facteurs.') }}
                    </p>
                </div>
                ...
                @if ($showingRecoveryCodes)
                    <x-confirms-password wire:then="regenerateRecoveryCodes">
                        <x-secondary-button class="mr-3">
                            {{ __('Générer de nouveaux codes de récupération') }}
                        </x-secondary-button>
                    </x-confirms-password>
